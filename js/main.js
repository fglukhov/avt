...
var baseUrl = "http://test.fglukhov.tmweb.ru/avt/";
...
$(document).ready(function () {
	...
	$(".calc-form-top-inner input").on("change input", function () {
	...
	$("#callbackForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				data: {
					subject: "АВТ - Заявка на обратный звонок",
	...
	$("#orderForm1").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: "АВТ - Заявка на консультацию",
					type: "Уточненная декларация без рисков",
					messenger: form.find(".form-messengers").attr("messenger"),
					messengerid: form.find(".form-messengers .form-group.active input").val()
				}
			}).done(function() {
	...
	$("#problemForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: "АВТ - Решение проблемы с госорганами",
	...
	$("#protectForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: "АВТ - Заявка на защиту бизнеса",
	...
	$("#calcForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: "АВТ - Заявка на расчет стоимости",
					type: "Калькулятор",
					pack: $(".calc-tabs li.active").html(),
					name: $("#calc_name").val(),
					messenger: form.find(".form-messengers").attr("messenger"),
					messengerid: form.find(".form-messengers .form-group.active input").val(),
					sum: $("#calc_sum").val()
	...
	$("a[data-tab]").click(function () {
		...
		$(".navbar-nav a[href='#calc']").click();

		$("[data-target='" + $(this).data("tab") + "']").click();
...
function validateForms() {
	...
	$("select").not("#calc_tabs_select").on("change", function () {
...
function formSuccess(form) {
	...
	form.find(".form-group input:not(.calc-form-top input), .form-group textarea").val("");
...
function ndsCalc() {
	...
	if ($(".calc-tabs li[data-target='#calc_rates_1']").hasClass("active")) {

		var ratesArr = [0.025,0.018,0.017,0.014];

	} else if ($(".calc-tabs li[data-target='#calc_rates_2']").hasClass("active")) {

		var ratesArr = [0.035,0.03,0.028,0.025];

	} else if ($(".calc-tabs li[data-target='#calc_rates_3']").hasClass("active")) {

		var ratesArr = [0.035,0.03,0.03,0.025];
	...
	if (totalVal < 5000000) {
		...
		priceVal = totalVal*ratesArr[0]* priceFactor;

	} else if (totalVal < 15000000) {

		priceVal = totalVal*ratesArr[1]* priceFactor;

	} else if (totalVal < 30000000) {

		priceVal = totalVal*ratesArr[2]* priceFactor;

	} else {

		priceVal = totalVal*ratesArr[3]* priceFactor;
...
function calcTabs() {
	...
	var curTab = $($(".calc-tabs li.active").data("target"));
	...
	$(".calc-tabs li").click(function () {
		...
		if (!$(this).hasClass("active")) {
			...
			var curTab = $($(this).data("target"));

			$(".calc-rates-tab").hide();

			curTab.show();

			$(".calc-tabs li").removeClass("active");

			$(this).addClass("active");

			ndsCalc();

			$("#calc_tabs_select").val($(this).data("target")).selectpicker("refresh");

		}

	});

	$("#calc_tabs_select").on("change", function () {

		$(".calc-tabs li[data-target='" + $(this).val() +"']").click();
...
function messengerTabs() {
	...
	$(".form-messengers-list li.active").each(function () {

		var curTab = $(this).closest(".form-messengers").find($(".form-messengers-field .form-group[data-index='" + $(this).data("index") + "']"));

		$(this).closest(".form-messengers").find(".form-messengers-field .form-group").hide().removeClass("active");

		curTab.show().addClass("active");

		$(this).closest(".form-messengers").find(".form-messengers-field input").attr("disabled", true);

		curTab.find("input").attr("disabled", false);

		if (curTab.find("input").attr("name").includes("telegram")) {

			$(this).closest(".form-messengers").attr("messenger", "Telegram");

		}

		if (curTab.find("input").attr("name").includes("wickr")) {

			$(this).closest(".form-messengers").attr("messenger", "Wickr");

		}

		if (curTab.find("input").attr("name").includes("signal")) {

			$(this).closest(".form-messengers").attr("messenger", "Signal");

		}

		if (curTab.find("input").attr("name").includes("email")) {

			$(this).closest(".form-messengers").attr("messenger", "Email");
	...
	$(".form-messengers-list li").click(function () {
		...
		if (!$(this).hasClass("active")) {
			...
			$(".form-messengers-list li").removeClass("active");
